...
def death_game(alcohol_game):
    ...
    picked_num = game.pick_num()
    choose_status = game.choose_someone()

    loser = game.find_loser(picked_num, choose_status)
...
class Death_game:
    def __init__(self, alcohol_game):
        ...
        self.players_name = self.get_players_name()
    ...
    def choose_someone(self): # 각 player들이 지목할 상대를 결정하는 기능
        ...
        for me, you in choose_status.items():
            print(f"{me} 👉 {you}")

        print("")
        time.sleep(1)

        return choose_status # Dictionary


    def find_loser(self, num=0, choose_status={}): # 게임 진행
        current_turn = self.tagger.name # 지목 turn 이름, 술래로 초기화

        for n in range(1, num+2):
            if n == num+1: # 마지막 num번째로 지목 당하게 된 loser case
                print(f"{current_turn} : 🤮")
                time.sleep(1)
                return current_turn # String(loser 이름)

            print(f"{current_turn} : {n}! 😎👉 {choose_status[current_turn]}")
            current_turn = choose_status[current_turn] # 지목 당한 사람을 다음 지목 turn으로 변경
            time.sleep(0.5)


    def get_players_name(self): # User 객체들의 이름 리스트를 생성하는 기능
        players_name = []
        for player in self.players:
            players_name.append(player.name)

        return players_name # List
